...
4. **Quote or intent custody**: `x402_check` cannot obtain requirements or a
   paid result has no executable `intentId`.
...
- For a paid check without an `intentId`, stop and preserve the same request;
  do not invent an intent ID.
