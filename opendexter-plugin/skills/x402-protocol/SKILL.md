...
- A purchasable paid result carries `quoteOnly=false` and one server-owned
  opaque `intentId` without executing it. A `quoteOnly=true` result carries no
  executable intent; report that purchase is unavailable for the checked quote
  and never call `x402_fetch` from it.
...
- `x402_check` discovers requirements and returns a server-owned `intentId`
  only when the checked request is purchasable.
