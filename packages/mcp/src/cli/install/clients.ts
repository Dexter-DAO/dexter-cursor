...
export function getClientConfig(client: ClientId, dev: boolean): ClientConfig {
  ...
  switch (client) {
    ...
    case "claude-code":
      ...
      // runInstall() intercepts claude-code and uses `claude mcp add` for the
      // local stdio server instead. This case exists only so the ClientId
      // switch is exhaustive; keep it inert.
      return {
