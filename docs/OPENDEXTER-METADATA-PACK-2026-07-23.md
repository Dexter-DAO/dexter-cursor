...
These four fields (`description`, `shortDescription`, `longDescription`, `defaultPrompt`) are **owned by the package manifest** — `plugins/opendexter/.codex-plugin/plugin.json`. To avoid two divergent copies of the same strings (that is exactly the drift Rule 7 forbids), read the current dashboard fields **from the manifest**, not from the dated inline draft below. Read them with: `python3 -c "import json;m=json.load(open('plugins/opendexter/.codex-plugin/plugin.json'));print(m['description']);print(m['interface']['shortDescription']);print(m['interface']['longDescription']);print(m['interface']['defaultPrompt'])"`.
